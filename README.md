...
The first field is the minute past the hour, the second field is the hour of the day and the third is the command to run.
For both cases * means that it should run for all values of that field. In the above example run_me_daily has been set to run at 1:30am every day and run_me_hourly at 45 minutes past the hour every hour.
...
You can submit your solution in any language you like, e.g. Python, PHP, C, JavaScript, Go, Rust, Swift, Awk, Prolog, using the standard libraries that are part of that language (i.e. no 3rd party libraries for this task please!). The choice is yours as long as we can run it as described below.
